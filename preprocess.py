...
result = pd.read_csv('sensor_clean.csv')
...
print("\n30분 단위 혼잡 비율 (%):")
result['time_slot'] = result['hour'].astype(str).str.zfill(2) + ':' + \
                      (result['minute'] // 30 * 30).astype(str).str.zfill(2)

pivot_30 = pd.crosstab(result['time_slot'], result['status_kor'], normalize='index') * 100
print(pivot_30.round(1))
