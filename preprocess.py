...
df_fix = pd.read_csv('sensor_clean.csv', encoding='utf-8-sig')

# status_kor 값 확인
print("현재 status_kor 값:")
print(df_fix['status_kor'].unique())
...
df_fix['status_kor'] = df_fix['status_kor'].map(
    lambda x: fix_map.get(x, x)
)

print("복구 후 status_kor 값:")
print(df_fix['status_kor'].unique())

# UTF-8로 다시 저장
df_fix.to_csv('sensor_clean.csv', index=False, encoding='utf-8-sig')
print("✅ 인코딩 복구 완료!")